-- ═══════════════════════════════════════════════════════════════════════════
-- GORR COMPLETE BATTLE PATCH
-- Paste seluruh file ini ke Supabase SQL Editor → Run
-- Mengubah: hero_skills (INSERT) + rpc_simulate_battle (full replace)
-- TIDAK mengubah: Lucas, Emma, semua slime — semua tetap persis sama
-- ═══════════════════════════════════════════════════════════════════════════

-- ─── STEP 1: hero_skills — hapus lama, insert benar ─────────────────────────
DELETE FROM hero_skills WHERE hero_id = 'gorr';

-- skill_id wajib NOT NULL — pakai gen_random_uuid()
INSERT INTO hero_skills (skill_id, hero_id, skill_slot, name, skill_type, damage_ratio, damage_type, target_type, unlock_level)
VALUES
  (gen_random_uuid(), 'gorr', 0, 'Savage Strike',    'damage', 1.00, 'physical', 'single',     1),
  (gen_random_uuid(), 'gorr', 1, 'Cleaver Rush',     'damage', 1.40, 'physical', 'single',     1),
  (gen_random_uuid(), 'gorr', 2, 'Intimidating Slam','damage', 1.58, 'physical', 'single',    21),
  (gen_random_uuid(), 'gorr', 4, 'Reaping Arc',      'damage', 0.88, 'physical', 'all_enemies', 61);
-- slot 3 (Bloodlust passive) di-hardcode di fungsi — tidak di hero_skills

-- ─── STEP 2: Full function replacement ──────────────────────────────────────
...
DECLARE
  ...
  v_sdef       RECORD;
  v_expanded   TEXT[];
  -- ── NEW: Gorr Blood Feast passive (Bleed tick + lifesteal) ────────────────
  v_gorr_lv3   INT;      -- passive level 0=locked 1-4
  v_dmg_total  INT;      -- total damage dealt this turn (for lifesteal)
  v_bleed_k    TEXT;     -- unit key while iterating bleed targets
  v_bleed_u    JSONB;    -- unit snapshot during bleed iteration
  v_bleed_dm   INT;      -- bleed tick damage for one target
  v_bleed_hl   INT;      -- lifesteal heal from bleed tick
  v_bleed_hp   INT;      -- gorr hp during bleed calc
  v_bleed_died BOOLEAN;  -- did bleed target die
  v_bleed_tj   JSONB;    -- targets array for bleed event
BEGIN
  ...
  LOOP
    ...
    LOOP
      ...
      -- ══════════════════════════════════════════════════════════════════════
      -- BLEED TICK — fires at START of each BLEEDING unit's own turn (2T)
      -- Actor in event = Gorr (bleed applier). Gorr receives lifesteal heal.
      -- Bleed bypasses shield and DEF (pure DoT). If victim dies → skip action.
      -- ══════════════════════════════════════════════════════════════════════
      IF COALESCE((v_actor->>'bleed_turns')::INT,0)>0
        ...
        END IF;
        -- Find alive Gorr on opposite side (the bleed applier)
        v_tuid:=NULL;
        FOR v_k IN SELECT k FROM jsonb_object_keys(v_units) AS t(k) LOOP
          ...
          END IF;
        END LOOP;
        -- Lifesteal heal for Gorr (Bloodlust passive): 8/10/13/17% per heroGallery
        v_bleed_hl:=0;
        ...
        IF v_bleed_hl>0 AND v_tuid IS NOT NULL THEN
          v_bleed_tj:=v_bleed_tj||jsonb_build_array(jsonb_build_object(
            ...
            'hp_after',(v_units->v_tuid->>'hp')::INT,
            'died',false,
        ...
        v_events:=v_events||jsonb_build_array(jsonb_build_object(
          't',v_eidx,
          'type','skill',
          'actor',COALESCE(v_tuid,v_auid),
          'hero_id','gorr','skill_slot',3,
        ...
        v_eidx:=v_eidx+1;
        IF v_bleed_died THEN CONTINUE inner; END IF; -- victim died, skip their action
      END IF;
      -- ── End Bleed tick ────────────────────────────────────────────────────
      ...
      -- (A) Bloodlust — lifesteal on every damage hit (8/10/13/17% per heroGallery)
      IF v_ahid='gorr' AND v_sktype='damage' THEN
        v_gorr_lv3:=CASE WHEN v_alv>=240 THEN 4
                         WHEN v_alv>=201 THEN 3
                         WHEN v_alv>=121 THEN 2
                         WHEN v_alv>=41  THEN 1 ELSE 0 END;
        IF v_gorr_lv3>0 THEN
      ...
      -- bleed_dmg per turn: 18/22/28/35% of Gorr's P.ATK (per heroGallery, level-scaled)
      -- Duration = 2 of TARGET's own turns
      IF v_ahid='gorr' AND v_slot=1 THEN
        v_gorr_lv3:=CASE WHEN v_alv>=240 THEN 4
                         WHEN v_alv>=201 THEN 3
                         WHEN v_alv>=121 THEN 2
                         WHEN v_alv>=41  THEN 1 ELSE 1 END;
...
-- ─── Verifikasi ─────────────────────────────────���────────────────────────────
SELECT skill_slot, name, skill_type, damage_ratio, damage_type, target_type, unlock_level
FROM hero_skills WHERE hero_id='gorr' ORDER BY skill_slot;