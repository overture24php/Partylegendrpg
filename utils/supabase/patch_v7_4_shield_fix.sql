-- ╔══════════════════════════════════════════════════════════════════════════════╗
-- ║  PATCH v7.4 — Shield system fix                                            ║
-- ║                                                                              ║
-- ║  Fixes:                                                                      ║
-- ║  1. All target events now include `shield_after` (remaining shield)         ║
-- ║     → Client knows exact shield remaining after every damage/heal/buff      ║
-- ║  2. Previously, damage events had no shield_after, so client shield bar     ║
-- ║     stayed at original value even after shield was consumed by damage       ║
-- ║                                                                              ║
-- ║  Includes all fixes from v7.1 + v7.2 + v7.3 (ATB integer fix).             ║
-- ║  Run in: Supabase Dashboard → SQL Editor → Run                              ║
-- ╚══════════════════════════════════════════════════════════════════════════════╝

-- (water_slime and emma fix already applied in v7.2/v7.3 — safe to re-run)
INSERT INTO public.hero_definitions
  (hero_id,name,rarity,hero_type,stars,base_hp,base_p_atk,base_m_atk,base_p_def,base_m_def,base_speed,growth_hp,growth_p_atk,growth_m_atk,growth_p_def,growth_m_def,sprite_url,illust_url,is_playable)
VALUES
('water_slime','Water Slime','common','Support',1,900,80,160,70,110,125,90,8,16,7,11,
 ...
 'https://res.cloudinary.com/dhkethrmc/image/upload/f_auto,q_auto/v1777545810/ChatGPT_Image_Apr_30_2026_05_40_35_PM_w370l3.png',false)
ON CONFLICT (hero_id) DO NOTHING;

INSERT INTO public.hero_skills
  (skill_id,hero_id,skill_slot,name,description,skill_type,damage_ratio,damage_type,effect_type,effect_duration,effect_chance,target_type,unlock_level,max_skill_level)
VALUES
('water_basic', 'water_slime',0,'Water Jet',  'Shoots a jet of water for 0.85× magical damage.','damage',0.850,'magical',NULL,0,0,'single',1,10),
('water_skill1','water_slime',1,'Bubble Heal','Heals the ally with lowest HP for 1.5× M.Atk.',  'heal',  1.500,'magical',NULL,0,0,'single_ally_hp',1,10),
('water_skill2','water_slime',2,'Tidal Wave', 'Deals 0.7× magical damage to all enemies.',       'damage',0.700,'magical',NULL,0,0,'all_enemies',5,10),
('water_ult',   'water_slime',3,'Tsunami',    'Massive wave — 2.0× magical damage to all enemies.','damage',2.000,'magical',NULL,0,0,'all_enemies',10,10)
ON CONFLICT (skill_id) DO NOTHING;

UPDATE public.hero_skills SET target_type='single_ally_hp'             WHERE skill_id='emma_skill1';
UPDATE public.hero_skills SET target_type='hp_shield_self',skill_type='buff' WHERE skill_id='rock_skill2';
...
BEGIN
  ...
  -- Load hero units (stats always computed fresh)
  ...
  -- ATB simulation (integer micro-ticks, CEIL division — no FP precision bug)
  <<outer>>
  LOOP
    v_iter:=v_iter+1;
    ...
    LOOP
      ...
      v_slot:=0; v_sk:=NULL;
      IF v_arage>=100 THEN
        SELECT s INTO v_sk FROM jsonb_array_elements(v_skills) AS t(s) WHERE (s->>'hero_id')=v_ahid AND (s->>'slot')::INT=3 AND (s->>'unlock_lv')::INT<=v_alv ORDER BY (s->>'unlock_lv')::INT DESC LIMIT 1;
        IF FOUND AND v_sk IS NOT NULL THEN v_slot:=3; END IF;
      END IF;
      IF v_sk IS NULL AND v_arage>=60 AND NOT v_ask2 THEN
        SELECT s INTO v_sk FROM jsonb_array_elements(v_skills) AS t(s) WHERE (s->>'hero_id')=v_ahid AND (s->>'slot')::INT=2 AND (s->>'unlock_lv')::INT<=v_alv ORDER BY (s->>'unlock_lv')::INT DESC LIMIT 1;
        IF FOUND AND v_sk IS NOT NULL THEN v_slot:=2; END IF;
      END IF;
      IF v_sk IS NULL AND v_arage>=30 AND NOT v_ask1 THEN
        SELECT s INTO v_sk FROM jsonb_array_elements(v_skills) AS t(s) WHERE (s->>'hero_id')=v_ahid AND (s->>'slot')::INT=1 AND (s->>'unlock_lv')::INT<=v_alv ORDER BY (s->>'unlock_lv')::INT DESC LIMIT 1;
        IF FOUND AND v_sk IS NOT NULL THEN v_slot:=1; END IF;
      END IF;
      IF v_sk IS NULL THEN
        SELECT s INTO v_sk FROM jsonb_array_elements(v_skills) AS t(s) WHERE (s->>'hero_id')=v_ahid AND (s->>'slot')::INT=0 AND (s->>'unlock_lv')::INT<=v_alv ORDER BY (s->>'unlock_lv')::INT DESC LIMIT 1;
        v_slot:=0;
      END IF;
      IF v_sk IS NULL THEN
        ...
        v_slot:=0;
      END IF;
      v_skname :=v_sk->>'name'; v_sktype:=v_sk->>'type'; v_skratio:=(v_sk->>'ratio')::NUMERIC;
      v_skdtype:=v_sk->>'dtype'; v_skttype:=v_sk->>'ttype';
        ...
        IF v_llv3>0 THEN
          ...
          IF FOUND AND v_tuid IS NOT NULL THEN
            v_units:=jsonb_set(v_units,ARRAY[v_tuid,'pds'],
              to_jsonb(LEAST(80,(v_units->v_tuid->>'pds')::INT+CASE v_llv3 WHEN 4 THEN 30 WHEN 3 THEN 25 WHEN 2 THEN 20 ELSE 15 END)));
          END IF;
        END IF;
      END IF;
      ...
      CASE
        ...
        WHEN v_skttype IN('self','hp_shield_self') THEN v_tuids:=ARRAY[v_auid];
        WHEN v_skttype='all_enemies' THEN
          SELECT array_agg(k) INTO v_tuids FROM jsonb_object_keys(v_units) AS t(k) WHERE (v_units->k)->>'side'=v_eside AND ((v_units->k)->>'alive')::BOOLEAN;
        ...
        WHEN v_skttype='two_front_random' THEN
          SELECT array_agg(k) INTO v_tuids FROM (SELECT k FROM jsonb_object_keys(v_units) AS t(k)
          WHERE (v_units->k)->>'side'=v_eside AND ((v_units->k)->>'alive')::BOOLEAN
            AND floor(((v_units->k)->>'slot')::NUMERIC/2)=(SELECT MAX(floor(((v_units->k2)->>'slot')::NUMERIC/2)) FROM jsonb_object_keys(v_units) AS t2(k2) WHERE (v_units->k2)->>'side'=v_eside AND ((v_units->k2)->>'alive')::BOOLEAN)
        ...
        WHEN v_skttype='highest_speed' THEN
          SELECT ARRAY[k] INTO v_tuids FROM jsonb_object_keys(v_units) AS t(k) WHERE (v_units->k)->>'side'=v_eside AND ((v_units->k)->>'alive')::BOOLEAN ORDER BY ((v_units->k)->>'speed')::INT DESC LIMIT 1;
        WHEN v_skttype='all_allies' THEN
          SELECT array_agg(k) INTO v_tuids FROM jsonb_object_keys(v_units) AS t(k) WHERE (v_units->k)->>'side'=v_alside AND ((v_units->k)->>'alive')::BOOLEAN;
        WHEN v_skttype='single_ally_hp' THEN
          SELECT ARRAY[k] INTO v_tuids FROM jsonb_object_keys(v_units) AS t(k) WHERE (v_units->k)->>'side'=v_alside AND ((v_units->k)->>'alive')::BOOLEAN ORDER BY ((v_units->k)->>'hp')::NUMERIC/GREATEST(1,((v_units->k)->>'max_hp')::NUMERIC) LIMIT 1;
        WHEN v_skttype='single_ally_maxhp' THEN
          SELECT ARRAY[k] INTO v_tuids FROM jsonb_object_keys(v_units) AS t(k) WHERE (v_units->k)->>'side'=v_alside AND ((v_units->k)->>'alive')::BOOLEAN ORDER BY ((v_units->k)->>'max_hp')::INT DESC LIMIT 1;
        ELSE
          IF v_sktype='buff' THEN v_tuids:=ARRAY[v_auid];
          ELSE
            SELECT ARRAY[k] INTO v_tuids FROM jsonb_object_keys(v_units) AS t(k)
            WHERE (v_units->k)->>'side'=v_eside AND ((v_units->k)->>'alive')::BOOLEAN
              AND floor(((v_units->k)->>'slot')::NUMERIC/2)=(SELECT MAX(floor(((v_units->k2)->>'slot')::NUMERIC/2)) FROM jsonb_object_keys(v_units) AS t2(k2) WHERE (v_units->k2)->>'side'=v_eside AND ((v_units->k2)->>'alive')::BOOLEAN)
      ...
      -- Lucas ult 3-hit expansion
      IF v_ahid='lucas' AND v_slot=3 THEN
        v_ultratio:=v_skratio/3.0; v_expanded:=ARRAY[]::TEXT[];
        FOREACH v_tuid IN ARRAY v_tuids LOOP v_expanded:=v_expanded||ARRAY[v_tuid,v_tuid,v_tuid]; END LOOP;
      ...
      FOREACH v_tuid IN ARRAY COALESCE(v_tuids,ARRAY[]::TEXT[]) LOOP
        ...
        IF v_sktype='damage' THEN
          ...
          -- Shield absorbs damage first (shield_after always included in event)
          IF v_tshield>0 THEN
          ...
          -- ── shield_after included so client always knows remaining shield ──
        ...
        ELSIF v_sktype='heal' THEN
          ...
          v_units:=jsonb_set(v_units,ARRAY[v_tuid,'hp'],to_jsonb(v_thp));
          -- Shield unchanged by heal — still include shield_after for consistency
          v_tj:=v_tj||jsonb_build_array(jsonb_build_object(
        ...
        ELSE -- buff/shield grant
          ...
          v_units:=jsonb_set(v_units,ARRAY[v_tuid,'shield'],to_jsonb(v_tshield));
          -- shield = granted amount this turn; shield_after = new total
          v_tj:=v_tj||jsonb_build_array(jsonb_build_object(
  ...
  IF v_winner='hero' THEN
    SELECT gold_reward,gem_reward,exp_reward,hero_exp_reward INTO v_sdef FROM stage_definitions WHERE stage_id=p_stage_id;
    IF FOUND THEN
      BEGIN v_snum:=split_part(p_stage_id,'-',2)::INT; EXCEPTION WHEN OTHERS THEN v_snum:=0; END;
      UPDATE profiles SET
        gold=gold+COALESCE(v_sdef.gold_reward,0),gems=gems+COALESCE(v_sdef.gem_reward,0),
        xp=xp+COALESCE(v_sdef.exp_reward,0),hero_exp=hero_exp+COALESCE(v_sdef.hero_exp_reward,0),
        chapter1_progress=CASE WHEN split_part(p_stage_id,'-',1)='1' THEN GREATEST(chapter1_progress,v_snum) ELSE chapter1_progress END
      WHERE id=v_uid;
      v_rewards:=jsonb_build_object('gold',COALESCE(v_sdef.gold_reward,0),'gems',COALESCE(v_sdef.gem_reward,0),'exp',COALESCE(v_sdef.exp_reward,0),'hero_exp',COALESCE(v_sdef.hero_exp_reward,0));
    END IF;
  END IF;
