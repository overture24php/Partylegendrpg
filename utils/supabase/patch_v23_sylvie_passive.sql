-- ╔══════════════════════════════════════════════════════════════════════════════╗
-- ║  PATCH v23 — Sylvie Swift Footing passive_init in rpc_simulate_battle      ║
-- ║                                                                              ║
-- ║  This patch completes Sylvie's passive (Swift Footing: Speed +12–26%).     ║
-- ║  sylvie_sql_patch.sql handled hero_definitions, hero_skills, gacha pool.   ║
-- ║  This patch handles the ONE remaining piece: the function logic.            ║
-- ║                                                                              ║
...
BEGIN
  ...
  -- ── Emma Blessed Ward passive_init: pre-battle team shield ────────────────
  v_shv:=0;
  ...
  -- ── Per-unit stat passives: Rock/Acid/Water Slime + Sylvie Swift Footing ──
  FOR v_k IN SELECT k FROM jsonb_object_keys(v_units) AS t(k) LOOP
    v_u:=v_units->v_k;
    v_llv3:=CASE WHEN (v_u->>'level')::INT>=240 THEN 4
                 WHEN (v_u->>'level')::INT>=201 THEN 3
    ...
    IF (v_u->>'hero_id')='water_slime' THEN
      v_units:=jsonb_set(v_units,ARRAY[v_k,'m_atk'],
        to_jsonb(GREATEST(1,round((v_units->v_k->>'m_atk')::NUMERIC*
          (1+CASE v_llv3 WHEN 1 THEN 0.08 WHEN 2 THEN 0.14 WHEN 3 THEN 0.20 ELSE 0.28 END))::INT)));
    END IF;
    -- ── v23 ADD: Sylvie Swift Footing — Speed +12/15/20/26% ─────────────────
  ...
  LOOP
    ...
    LOOP
      ...
      -- ── HOT TICK — fires before actor's action if unit has HoT ──────────────
...
-- ── Verify ────────────────────────────────────────────────────────────────────
-- Confirm the passive section is in place:
SELECT pg_get_functiondef(oid) LIKE '%sylvie%' AS sylvie_passive_present
FROM pg_proc WHERE proname = 'rpc_simulate_battle'
