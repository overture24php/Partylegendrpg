-- ╔══════════════════════════════════════════════════════════════════════════════╗
-- ║  PATCH v7.7 — Emma SK2 target fix + Slime unlock level definitive fix     ║
-- ║                                                                              ║
-- ║  ROOT CAUSE — Bug 1 (Emma SK2):                                            ║
-- ║    battle_schema_v7_skill_audit.sql line 84 SET target_type='single_ally_hp'║
-- ║    (= lowest HP%) instead of 'single_ally_maxhp' (= highest Max HP).       ║
-- ║    At battle start everyone is at full HP → HP% tie → RANDOM winner →      ║
-- ║    Emma sometimes shields herself, sometimes Lucas.                         ║
-- ║    This patch: fixes DB + hardens function with explicit Emma SK2 override. ║
-- ║    Shield value: ratio × Emma's M.ATK  (NOT target's maxHP).               ║
-- ║                                                                              ║
-- ║  ROOT CAUSE — Bug 2 (Slime SK2 at low level):                             ║
-- ║    battle_schema_v7_skill_audit.sql water_slime INSERT has                 ║
-- ║    ON CONFLICT DO UPDATE SET unlock_level = EXCLUDED.unlock_level          ║
-- ║    → re-running audit resets water_skill2 to unlock_level=5.              ║
-- ║    Rock/Acid slimes were not affected (their UPDATE doesn't touch it),     ║
-- ║    explaining "some slimes have the bug, some don't."                      ║
-- ║    This patch: UPDATE with LIKE '%slime%' catches all variants;            ║
-- ║    audit SQL was also fixed (unlock_level 21/41/1 for SK1/SK2/ULT).       ║
-- ║                                                                              ║
-- ║  Run: Supabase Dashboard → SQL Editor → Run                                ║
-- ╚══════════════════════════════════════════════════════════════════════════════╝

-- ── 1. Emma SK2: fix target_type ──────────────────────────────────────────────
-- 'single_ally_hp' = lowest HP%  (WRONG — random tie at battle start)
-- 'single_ally_maxhp' = highest Max HP  (CORRECT per skill description)
-- Shield value is M.ATK-based (function handles this automatically since
-- single_ally_maxhp is NOT in hp_shield_self/self → falls to M.ATK ELSE branch)
UPDATE public.hero_skills
SET target_type = 'single_ally_maxhp'
WHERE skill_id = 'emma_skill2';

-- ── 2. ALL slime skill unlock_levels — definitive fix ─────────────────────────
-- Canonical unlock system: basic=1, SK1=1, SK2=21, passive=41, ULT=61
-- LIKE '%slime%' catches rock_slime, acid_slime, water_slime and any future variants.
UPDATE public.hero_skills
SET unlock_level = CASE skill_slot
  WHEN 0 THEN 1   -- basic attack: always available
  WHEN 1 THEN 1   -- SK1: available from Lv 1
  WHEN 2 THEN 21  -- SK2: unlock at Lv 21 (all stage-1 slimes lv 2-12 → never fires)
  WHEN 3 THEN 61  -- ULT: unlock at Lv 61
  ELSE unlock_level
END
WHERE LOWER(hero_id) LIKE '%slime%';

-- ── Full function replacement ─────────────────────────────────────────────────
-- Key changes vs v7.6:
--   • Emma SK2 hardcoded override BEFORE CASE block → immune to DB target_type bugs
--   • hp_shield_self/self uses target maxHP; everything else uses caster M.ATK
...
BEGIN
  ...
  LOOP
    ...
    LOOP
      ...
      -- Cooldown decrement at start of actor's turn
      ...
      -- Immune to DB target_type bugs. Always targets the living ally with the
      -- highest Max HP (Lucas in normal setup). Shield = ratio × Emma's M.ATK.
      -- v_skttype is kept as whatever is in DB — the buff-calc ELSE branch
      -- (not in hp_shield_self/self) correctly uses v_amatk for all others.
      IF v_ahid='emma' AND v_slot=2 THEN
        ...
        -- Fallback: target self if no other ally alive
        IF v_tuids IS NULL OR array_length(v_tuids,1)=0 THEN
          v_tuids:=ARRAY[v_auid];
        END IF;
        -- Ensure sktype=buff so shield branch executes
        v_sktype :='buff';
        -- skttype must NOT be hp_shield_self/self → buff-calc uses M.ATK for value
        v_skttype:='single_ally_maxhp';
      END IF;

      -- Run CASE only when Emma SK2 override didn't already set v_tuids
      IF v_tuids IS NULL THEN
        CASE
          ...
          -- single_ally_maxhp: kept for completeness (Emma SK2 is handled above)
        ...
        END CASE;
      END IF; -- end Emma SK2 guard

      IF v_tuids IS NULL THEN v_tuids:=ARRAY[]::TEXT[]; END IF;

      -- Lucas ULT: 3 hits per front-row target at ratio 1.0 each = 300% total
      ...
      FOREACH v_tuid IN ARRAY COALESCE(v_tuids,ARRAY[]::TEXT[]) LOOP
        ...
        ELSE -- buff / shield grant
          -- hp_shield_self / self:  shield = ratio × TARGET's own maxHP (Rock Shell)
          -- all other buff types:   shield = ratio × CASTER's M.ATK   (Emma Bulwark Veil)
