-- ╔══════════════════════════════════════════════════════════════════════════════╗
-- ║  PATCH v7.3 — ATB Floating-Point Precision Fix                             ║
-- ║                                                                              ║
-- ║  Root Cause:                                                                 ║
-- ║    PostgreSQL NUMERIC arithmetic: 147 × (1000 / 147) = 999.999999…         ║
-- ║    (repeating decimal, never exactly 1000)                                  ║
-- ║    → No unit ever satisfies `ap >= 1000`                                    ║
-- ║    → v_iter hits 2000 after just 2 hero actions (Emma + Lucas)              ║
-- ║    → Both sides alive → winner = 'enemy' by default                         ║
-- ║                                                                              ║
-- ║  Fix:                                                                        ║
-- ║    After each skip-time advance, if no unit has ap >= v_atb,                ║
-- ║    snap the highest-AP alive unit to exactly v_atb.                         ║
-- ║    Also: use ap >= v_atb - 0.5 tolerance in actor selection.                ║
-- ║    This completely eliminates the micro-epsilon infinite loop.               ║
-- ║                                                                              ║
-- ║  Includes all fixes from v7.1 + v7.2.                                       ║
-- ║  Run in: Supabase Dashboard → SQL Editor → Run                              ║
-- ╚══════════════════════════════════════════════════════════════════════════════╝

-- ── Ensure water_slime exists ────────────────────────────────────────────────
...
-- ── Recreate with ATB precision fix ─────────────────────────────────────────
...
DECLARE
  ...
  v_inh        JSONB    := '{}'::JSONB;
  v_inmh       JSONB    := '{}'::JSONB;
  -- ATB: use integer micro-ticks (×1000) to avoid NUMERIC repeating-decimal issues
  -- All AP stored as integers: 0..1000000 (representing 0.000..1000.000)
  -- speed integer × advance_ticks → no float division rounding
  v_atb        BIGINT   := 1000000;   -- 1000 × 1000
  v_iter       INT      := 0;
  v_eidx       INT      := 0;
  v_rec        RECORD;
  v_i          INT;
...
BEGIN
  ...
  -- ════════════════════════════════════════════════════════════════════════════
  -- NORMALIZE p_hero_entries (string vs array safety)
  -- ════════════════════════════════════════════════════════════════════════════
  ...
  -- ════════════════════════════════════════════════════════════════════════════
  -- LOAD HERO UNITS
  -- Stats always computed fresh from hero_definitions (cheat-proof + resilient)
  -- AP stored as BIGINT (speed × 1000 per time-unit tick)
  -- ════════════════════════════════════════════════════════════════════════════
  ...
  LOOP
    ...
    v_units := v_units || jsonb_build_object(
      v_uuid_str, jsonb_build_object(
        ...
        'speed',v_rec.speed,
        -- AP as BIGINT micro-ticks (speed × 1000 per unit time)
        'ap', 0,
  ...
  LOOP
    v_skills := v_skills || jsonb_build_array(jsonb_build_object(
      ...
      'dtype',v_rec.damage_type,'ttype',v_rec.target_type,
  ...
  -- ATB SIMULATION LOOP
  --
  -- KEY FIX: AP stored and compared as BIGINT micro-ticks.
  --   v_atb  = 1000000  (= 1000 × 1000)
  --   speed  = e.g. 147 (integer stored in unit)
  --   advance_ticks = CEIL( (v_atb - ap) / speed )  ← integer CEIL, exact!
  --   Each unit gains speed × advance_ticks AP per step.
  --   Because CEIL division is used, the fastest unit always reaches
  --   EXACTLY v_atb with no floating-point underrun.
  -- ════════════════════════════════════════════════════════════════════════════
  <<outer>>
  LOOP
    v_iter := v_iter + 1;
    ...
    LOOP
      ...
      -- Find highest-AP ready actor (>= v_atb); tiebreak by speed
      v_auid := NULL;
      FOR v_k IN SELECT k FROM jsonb_object_keys(v_units) AS t(k)
      LOOP
        v_u := v_units -> v_k;
      ...
      -- Deduct v_atb from actor's AP (keep overshoot for next turn)
      v_units := jsonb_set(v_units, ARRAY[v_auid,'ap'],
        to_jsonb((v_units->v_auid->>'ap')::BIGINT - v_atb));
      ...
      v_tuids := NULL;
      CASE
        -- Heal 'single' always → lowest-HP ally
        ...
        ELSE
          -- Default: single front enemy for damage/buff; self-fallback for buff
  ...
  -- WINNER
  -- ════════════════════════════════════════════════════════════════════════════
  v_halive:=EXISTS(SELECT 1 FROM jsonb_object_keys(v_units) AS t(k)
                   WHERE (v_units->k)->>'side'='hero' AND ((v_units->k)->>'alive')::BOOLEAN);
  v_ealive:=EXISTS(SELECT 1 FROM jsonb_object_keys(v_units) AS t(k)
                   WHERE (v_units->k)->>'side'='enemy' AND ((v_units->k)->>'alive')::BOOLEAN);
