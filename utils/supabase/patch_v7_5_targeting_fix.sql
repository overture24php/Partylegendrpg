-- ╔══════════════════════════════════════════════════════════════════════════════╗
-- ║  PATCH v7.5 — Front/Back targeting + Cooldown system + Lucas 3-hit ULT    ║
-- ║                                                                              ║
-- ║  TARGETING FIX:                                                             ║
-- ║    Old (WRONG): floor(slot/2) = row index (sprite size tier 0-2)           ║
-- ║    New (RIGHT): slot%2        = column depth (FRONT vs BACK)               ║
-- ║    Hero  side FRONT = col1 (slot%2=1, right col, nearest to enemies)       ║
-- ║    Enemy side FRONT = col0 (slot%2=0, left  col, nearest to heroes)        ║
-- ║                                                                              ║
-- ║  COOLDOWN SYSTEM (replaces rage gate for SK1/SK2):                         ║
-- ║    SK1: 2-turn cooldown  (decrement each time THIS unit acts)              ║
-- ║    SK2: 4-turn cooldown  (decrement each time THIS unit acts)              ║
-- ║    Both start at 0 (ready). Priority: SK1 before SK2 when both available. ║
-- ║    Rage ONLY controls ULT (≥100).                                          ║
-- ║                                                                              ║
-- ║  LUCAS ULT:                                                                 ║
-- ║    target_type = front_aoe (all alive front-col enemies)                   ║
-- ║    3 hits per target × 100% ratio each = 300% total per target             ║
-- ║    damage floats appear 3× per target with stagger delay (client-side)     ║
-- ║                                                                              ║
-- ║  Run: Supabase Dashboard → SQL Editor → Run                                ║
-- ╚══════════════════════════════════════════════════════════════════════════════╝

-- ── Skill data updates ────────────────────────────────────────────────────────
-- Lucas ULT: front_aoe, 1.0 per hit (× 3 expansion in sim = 300% total/target)
UPDATE public.hero_skills
SET target_type   = 'front_aoe',
    damage_ratio  = 1.0
WHERE hero_id = 'lucas' AND skill_slot = 3;
...
DECLARE
  ...
  v_alv        INT;
  v_arage      INT;
  -- ── Cooldown variables (replaces sk1_used/sk2_used) ─────────────────────
  v_sk1cd      INT;      -- turns until SK1 is ready (0 = ready)
  v_sk2cd      INT;      -- turns until SK2 is ready (0 = ready)
  -- ────────────────────────────────────────────────────────────────────────
  v_apatk      INT;
  v_amatk      INT;
  v_apds       INT;
  v_eside      TEXT;     -- target side (opposite of actor)
  v_alside     TEXT;     -- actor's own side
  -- ── Front/back column helpers ────────────────────────────────────────────
  v_front_col  INT;      -- slot%2 value = FRONT for v_eside
  v_back_col   INT;      -- slot%2 value = BACK  for v_eside
  -- ────────────────────────────────────────────────────────────────────────
  v_slot       INT;
  v_sk         JSONB;
  v_skname     TEXT;
  ...
  v_snum       INT;
  v_sdef       RECORD;
  v_expanded   TEXT[];   -- for Lucas ULT 3-hit expansion
BEGIN
  ...
  FOR v_i IN 0..jsonb_array_length(p_hero_entries)-1 LOOP
    ...
    v_units:=v_units||jsonb_build_object(v_uuid_str,jsonb_build_object(
      ...
      'ap',0,'rage',0,
      'sk1_cd',0,'sk2_cd',0,   -- both skills ready at battle start
  ...
  LOOP
    ...
    LOOP
      ...
      -- Pick actor with highest AP (tiebreak: higher speed)
      v_auid:=NULL;
      FOR v_k IN SELECT k FROM jsonb_object_keys(v_units) AS t(k) LOOP
        v_u:=v_units->v_k;
        IF ((v_u->>'alive')::BOOLEAN) AND ((v_u->>'ap')::BIGINT>=v_atb) THEN
      ...
      -- ── Front/back column for TARGET side ─────────────────────────────────
      -- hero  side front = col1 (slot%2=1, right column, nearest to enemies)
      -- enemy side front = col0 (slot%2=0, left  column, nearest to heroes)
      ...
      -- Read stale value from v_actor, apply -1, write back, store in locals
      ...
      -- ── Skill selection ───────────────────────────────────────────────────
      -- Priority: ULT (rage 100) > SK1 (cd=0) > SK2 (cd=0) > Basic
      -- SK1 goes before SK2 so at battle start they fire in order: SK1, SK2, ...
      v_slot:=0; v_sk:=NULL;

      -- 1. ULT — rage gate only
      IF v_arage>=100 THEN
      ...
      -- 2. SK1 — cooldown gate (priority over SK2 per user spec)
      ...
      -- ══════════════════════════════════════════════════════════════════════
      -- TARGET RESOLUTION — slot%2 based (v_front_col / v_back_col)
      -- FRONT = col nearest to opponent | BACK = col furthest from opponent
      -- Every branch has a fallback when the target column is empty.
      -- ══════════════════════════════════════════════════════════════════════
      v_tuids:=NULL;
      ...
      CASE
        ...
        -- FRONT AOE: all alive enemies in FRONT COLUMN
        WHEN v_skttype='front_aoe' THEN
          ...
          WHERE (v_units->k)->>'side'=v_eside
            ...
            AND ((v_units->k)->>'slot')::INT % 2 = v_front_col;
          -- Fallback: front col empty → hit all alive
          IF v_tuids IS NULL OR array_length(v_tuids,1)=0 THEN
            SELECT array_agg(k) INTO v_tuids FROM jsonb_object_keys(v_units) AS t(k)
            WHERE (v_units->k)->>'side'=v_eside AND ((v_units->k)->>'alive')::BOOLEAN;
          END IF;

        -- ALL BACK: all alive enemies in BACK COLUMN
        ...
        -- SINGLE BACK: 1 random from BACK COLUMN
        ...
        -- TWO FRONT RANDOM: 2 random from FRONT COLUMN
        ...
        -- TWO FRONT HIGHEST HP: 2 highest-HP from FRONT COLUMN
        WHEN v_skttype='two_front_highest_hp' THEN
          ...
          FROM (
            ...
            WHERE (v_units->k)->>'side'=v_eside
              AND ((v_units->k)->>'alive')::BOOLEAN
              AND ((v_units->k)->>'slot')::INT % 2 = v_front_col
            ORDER BY ((v_units->k)->>'max_hp')::INT DESC LIMIT 2
          ) sub;
          IF v_tuids IS NULL OR array_length(v_tuids,1)=0 THEN
            SELECT array_agg(k) INTO v_tuids
            FROM (SELECT k FROM jsonb_object_keys(v_units) AS t(k)
        ...
        ELSE
          ...
          ELSE
            ...
            WHERE (v_units->k)->>'side'=v_eside
              ...
              AND ((v_units->k)->>'slot')::INT % 2 = v_front_col
            ORDER BY ((v_units->k)->>'slot')::INT LIMIT 1;
            -- Fallback: front col empty → any alive enemy
            IF v_tuids IS NULL THEN
              SELECT ARRAY[k] INTO v_tuids FROM jsonb_object_keys(v_units) AS t(k)
              WHERE (v_units->k)->>'side'=v_eside AND ((v_units->k)->>'alive')::BOOLEAN
              ORDER BY ((v_units->k)->>'slot')::INT LIMIT 1;
      ...
      -- After front_aoe resolves v_tuids (e.g. [enemy-0, enemy-4]),
      -- expand each to 3 entries: [enemy-0, enemy-0, enemy-0, enemy-4, enemy-4, enemy-4]
      -- v_skratio = 1.0 per hit (set by DB update above) → 300% total per target
      ...
      FOREACH v_tuid IN ARRAY COALESCE(v_tuids,ARRAY[]::TEXT[]) LOOP
        -- Re-read latest state each hit (important for multi-hit like Lucas ULT)
      ...
      IF v_slot=3 THEN
        -- ULT used: reset rage, SK CDs unchanged
        v_arage:=0;
        v_units:=jsonb_set(v_units,ARRAY[v_auid,'rage'],to_jsonb(0));

      ELSIF v_slot=1 THEN
        -- SK1 used: set 2-turn CD, gain small rage
        v_arage:=LEAST(100,v_arage+5);
        v_units:=jsonb_set(v_units,ARRAY[v_auid,'rage'],to_jsonb(v_arage));
        -- sk1_cd=2 → decrements next 2 actions (1→0) before it's ready again
        v_units:=jsonb_set(v_units,ARRAY[v_auid,'sk1_cd'],to_jsonb(2));

      ELSIF v_slot=2 THEN
        -- SK2 used: set 4-turn CD, gain small rage
        v_arage:=LEAST(100,v_arage+5);
        v_units:=jsonb_set(v_units,ARRAY[v_auid,'rage'],to_jsonb(v_arage));
        -- sk2_cd=4 → decrements next 4 actions before ready again
        v_units:=jsonb_set(v_units,ARRAY[v_auid,'sk2_cd'],to_jsonb(4));

      ELSE
        -- Basic attack: gain rage toward ULT
        v_arage:=LEAST(100,v_arage+5);
        v_units:=jsonb_set(v_units,ARRAY[v_auid,'rage'],to_jsonb(v_arage));
      END IF;
  ...
  -- ── Determine winner & distribute rewards ─────────────────────────────────
