-- ╔══════════════════════════════════════════════════════════════════════════════╗
-- ║  PATCH v22 — Full Mechanics Fix (Fang / Clover / Myko)                       ║
-- ╠══════════════════════════════════════════════════════════════════════════════╣
-- ║  FIXED:                                                                       ║
-- ║  1. Myko Fungal Resilience — fires for ALL levels (removed lv≥41 gate)       ║
-- ║  2. Fang Twin Slash — twin_slash targeting → 2 consecutive hits              ║
-- ║  3. Fang Death Bound — single_lowest_hp target + ×3 execute <35% HP         ║
-- ║  4. Clover Life Bloom — reactive 35% chance heal on ally direct hit          ║
-- ║  5. Clover Lucky Toss — proper HoT (3 turns, 55%/turn), random alive ally   ║
-- ║  6. Clover Bloom Cascade — burst heal all + HoT (3T, 38%/turn) on all      ║
-- ║  7. Fang sk2BeforeSk1 + SK1 no-cooldown                                     ║
-- ║  8. Added: single_random_ally, single_lowest_hp_ally targeting              ║
-- ║                                                                               ║
-- ║  Run in: Supabase Dashboard → SQL Editor → Run                               ║
-- ╚══════════════════════════════════════════════════════════════════════════════╝

-- ════════════════════════════════════════════════════════════════════════════════
-- STEP 1: hero_skills DB fixes (idempotent)
-- ════════════════════════════════════════════════════════════════════════════════

-- Clover SK2: Lucky Toss — restore to HoT type, random ally target
UPDATE public.hero_skills SET
  skill_type   = 'hot',
  damage_ratio = 0.55,
  damage_type  = 'magical',
  target_type  = 'single_random_ally',
  unlock_level = 21
WHERE hero_id = 'clover' AND skill_slot = 2;

-- Clover ULT: Bloom Cascade — burst heal + HoT
UPDATE public.hero_skills SET
  skill_type   = 'heal_aoe',
  damage_ratio = 0.90,
  damage_type  = 'magical',
...
-- Clover SK3: Life Bloom passive — ensure correct type
UPDATE public.hero_skills SET
  skill_type   = 'passive',
  damage_ratio = 0.40,
  damage_type  = 'magical',
...
-- Clover SK1: ensure correct targeting
UPDATE public.hero_skills SET
  skill_type   = 'heal',
  damage_ratio = 1.30,
  damage_type  = 'magical',
  target_type  = 'single_lowest_hp_ally',
  unlock_level = 1
WHERE hero_id = 'clover' AND skill_slot = 1;

-- Fang SK1: Twin Slash — twin_slash + no cooldown (ratio 0.90 per hit)
UPDATE public.hero_skills SET
  damage_ratio = 0.90,
...
-- Fang ULT: Death Bound — single_lowest_hp + execute
UPDATE public.hero_skills SET
  damage_ratio = 2.60,
...
-- ════════════════════════════════════════════════════════════════════════════════
-- STEP 2: Replace rpc_simulate_battle (full replacement — includes all v13 features)
-- ════════════════════════════════════════════════════════════════════════════════
...
DECLARE
  ...
  v_edef       INT;
  v_dmg        INT;
  v_dmg_pre    INT;   -- pre-shield damage (used for passive checks like Myko Fungal Resilience)
  v_heal       INT;
  v_shv        INT;
  v_madv       BIGINT;
  v_adv        BIGINT;
  ...
  v_spore_tj   JSONB;
  -- ── NEW v22 ──────────────────────────────────────────────────────────────────
  -- HoT system (Clover Lucky Toss / Bloom Cascade)
  v_hot_map    JSONB    := '{}'::JSONB;  -- {uid: {turns:INT, ratio:NUMERIC, m_atk:INT}}
  v_hot_turns  INT;
  v_hot_ratio  NUMERIC;
  v_hot_matk   INT;
  v_hot_heal   INT;
  -- Reactive passive events (Myko regen, Clover Life Bloom)
  v_react_evts JSONB    := '[]'::JSONB;
  v_i2         INT;
  -- Clover Life Bloom passive vars
  v_k2         TEXT;
  v_clover_uid TEXT;
  v_clover_matk INT;
  v_clover_lv3 INT;
...
BEGIN
  ...
  -- ── Pre-battle passives ────────────────────────────────────────────────────
  -- Emma Blessed Ward: start-of-battle shield for hero team
  v_shv:=0;
  ...
  IF v_shv>0 THEN
    ...
    END LOOP;
  END IF;
  -- Per-unit stat passives: Rock/Acid/Water Slime
  FOR v_k IN SELECT k FROM jsonb_object_keys(v_units) AS t(k) LOOP
    v_u:=v_units->v_k;
    v_llv3:=CASE WHEN (v_u->>'level')::INT>=240 THEN 4
                 WHEN (v_u->>'level')::INT>=201 THEN 3
  ...
  -- Initialize per-hero tracking fields
  FOR v_k IN SELECT k FROM jsonb_object_keys(v_units) AS t(k) LOOP
    v_u:=v_units->v_k;
  ...
  -- Emit passive_init shields snapshot
  v_combj:='{}'::JSONB;
  FOR v_k IN SELECT k FROM jsonb_object_keys(v_units) AS t(k) LOOP
  ...
  -- Load skills for all hero_ids in battle
  FOR v_rec IN
    ...
    LOOP
      ...
      IF COALESCE((v_actor->>'myko_spore_turns')::INT,0)>0
        ...
        v_spore_tj:=jsonb_build_array(jsonb_build_object(
          'uid',v_auid,'dmg',v_spore_dm,'hp_after',v_spore_hp,'died',v_spore_died,
          'rage_after',(v_actor->>'rage')::INT,'shield_after',(v_actor->>'shield')::INT));
        v_events:=v_events||jsonb_build_array(jsonb_build_object(
          't',v_eidx,'type','skill',
      ...
      -- ── HOT TICK (v22) — fires before actor's action if unit has HoT ────────
      IF v_hot_map ? v_auid THEN
        v_hot_turns := (v_hot_map->v_auid->>'turns')::INT;
        IF v_hot_turns > 0 AND ((v_units->v_auid->>'alive')::BOOLEAN) THEN
          v_hot_ratio := (v_hot_map->v_auid->>'ratio')::NUMERIC;
          v_hot_matk  := (v_hot_map->v_auid->>'m_atk')::INT;
          v_hot_heal  := GREATEST(1, floor(v_hot_matk::NUMERIC * v_hot_ratio));
          v_hp        := (v_units->v_auid->>'hp')::INT;
          v_hp        := LEAST((v_units->v_auid->>'max_hp')::INT, v_hp + v_hot_heal);
          v_units     := jsonb_set(v_units, ARRAY[v_auid,'hp'], to_jsonb(v_hp));
          v_hot_turns := v_hot_turns - 1;
          IF v_hot_turns > 0 THEN
            v_hot_map := jsonb_set(v_hot_map, ARRAY[v_auid,'turns'], to_jsonb(v_hot_turns));
          ELSE
            v_hot_map := v_hot_map - v_auid;
          END IF;
          v_events := v_events || jsonb_build_array(jsonb_build_object(
            't',v_eidx,'type','skill',
            'actor',v_auid,'hero_id',v_ahid,'skill_slot',3,
            'skill_name','HoT','skill_type','hot','skill_dtype','magical',
            'targets',jsonb_build_array(jsonb_build_object(
              'uid',v_auid,'heal',v_hot_heal,'hp_after',v_hp,
              'hot_turns',v_hot_turns,'died',false,
              'rage_after',(v_units->v_auid->>'rage')::INT,
              'shield_after',(v_units->v_auid->>'shield')::INT)),
            'rage_after',(v_units->v_auid->>'rage')::INT,'lucas_stacks',NULL
          ));
          v_eidx:=v_eidx+1;
          EXIT outer WHEN v_eidx>=1500;
          -- Re-read actor after HoT
          IF NOT ((v_units->v_auid->>'alive')::BOOLEAN) THEN CONTINUE inner; END IF;
          v_actor:=v_units->v_auid;
          v_amatk:=(v_actor->>'m_atk')::INT;
        END IF;
      END IF;
      ...
      -- Fang sk2BeforeSk1: SK2 checked before SK1 when rage ≥ 60
      IF v_sk IS NULL AND v_ahid='fang' AND v_arage>=60 AND v_sk2cd=0 THEN
        SELECT s INTO v_sk FROM jsonb_array_elements(v_skills) AS t(s)
      ...
      IF v_sk IS NULL AND v_sk1cd=0 THEN
        ...
        ORDER BY (s->>'unlock_lv')::INT DESC LIMIT 1;
        IF FOUND AND v_sk IS NOT NULL THEN
          -- Skip passives in active slots
          IF (v_sk->>'type')='passive' THEN v_sk:=NULL;
          ELSE v_slot:=1; END IF;
        END IF;
      END IF;

      -- SK2 (cd=0, non-Fang or Fang rage<60)
      ...
      -- Lucas SK2: armor-rend debuff (pre-hit P.DEF shred)
      ...
      -- Emma SK2 hardcoded override (ally with highest MaxHP)
      ...
      IF v_tuids IS NULL THEN
        CASE
          -- Heal on lowest HP ally
          WHEN v_sktype='heal' AND v_skttype IN('single','single_lowest_hp_ally') THEN
            SELECT ARRAY[k] INTO v_tuids FROM jsonb_object_keys(v_units) AS t(k)
            WHERE (v_units->k)->>'side'=v_alside AND ((v_units->k)->>'alive')::BOOLEAN
          ...
          -- All alive allies (heal AoE / buffs)
          ...
          -- Twin Slash (Fang SK1) — single front enemy, duplicated for 2 hits
          WHEN v_skttype='twin_slash' THEN
            ...
            IF v_tuids IS NULL THEN
              ...
              ORDER BY ((v_units->k)->>'slot')::INT LIMIT 1;
            END IF;
            -- Duplicate target for 2 hits
            IF v_tuids IS NOT NULL AND array_length(v_tuids,1)=1 THEN
              v_tuids:=ARRAY[v_tuids[1],v_tuids[1]];
            END IF;

          -- Lowest current HP% enemy (Fang ULT)
          WHEN v_skttype='single_lowest_hp' THEN
            SELECT ARRAY[k] INTO v_tuids FROM jsonb_object_keys(v_units) AS t(k)
            WHERE (v_units->k)->>'side'=v_eside AND ((v_units->k)->>'alive')::BOOLEAN
            ORDER BY ((v_units->k)->>'hp')::NUMERIC/GREATEST(1,((v_units->k)->>'max_hp')::NUMERIC) ASC LIMIT 1;

          -- Lowest HP% ally (Clover SK1)
          WHEN v_skttype='single_lowest_hp_ally' THEN
            SELECT ARRAY[k] INTO v_tuids FROM jsonb_object_keys(v_units) AS t(k)
            WHERE (v_units->k)->>'side'=v_alside AND ((v_units->k)->>'alive')::BOOLEAN
            ORDER BY ((v_units->k)->>'hp')::NUMERIC/GREATEST(1,((v_units->k)->>'max_hp')::NUMERIC) ASC LIMIT 1;

          -- Random alive ally (Clover SK2 Lucky Toss)
          WHEN v_skttype='single_random_ally' THEN
            SELECT ARRAY[k] INTO v_tuids FROM jsonb_object_keys(v_units) AS t(k)
            WHERE (v_units->k)->>'side'=v_alside AND ((v_units->k)->>'alive')::BOOLEAN
            ORDER BY random() LIMIT 1;

          -- Passives: no auto-targeting
          WHEN v_skttype IN('passive','passive_ally_hit','passive_kill_stack','passive_init') THEN
            v_tuids:=ARRAY[]::TEXT[];
      ...
      -- ── Fang Death Bound execute bonus ×3 when target HP% < 35% ─────────────
      IF v_ahid='fang' AND v_slot=4 AND v_tuids IS NOT NULL AND array_length(v_tuids,1)>=1 THEN
        v_tuid := v_tuids[1];
        IF v_tuid IS NOT NULL AND ((v_units->v_tuid->>'alive')::BOOLEAN) THEN
          IF ((v_units->v_tuid->>'hp')::NUMERIC / GREATEST(1,(v_units->v_tuid->>'max_hp')::NUMERIC)) < 0.35 THEN
            v_skratio := v_skratio * 3.0;
          END IF;
        END IF;
      END IF;
      ...
      v_tj:='[]'::JSONB;
      v_craw_cr_now:=FALSE;
      v_react_evts:='[]'::JSONB;  -- reset reactive events for this turn
      ...
      FOREACH v_tuid IN ARRAY COALESCE(v_tuids,ARRAY[]::TEXT[]) LOOP
        ...
        IF v_sktype='damage' THEN
          ...
          IF v_ahid='lucas' AND v_skdtype='physical' THEN
            ...
            -- v_batk unchanged; Lucas' rage stacks compound over turns (handled separately)
          END IF;
          ...
          END IF;
          v_dmg_pre := v_dmg;  -- save pre-shield damage for passive triggers
          IF v_tshield>0 THEN
          ...
          -- ── Myko Fungal Resilience — reactive heal when hit (v22 fix) ──────────
          -- Uses v_dmg_pre (pre-shield) so shield-fully-absorbed hits still trigger.
          IF (v_tgt->>'hero_id')='myko' AND v_dmg_pre>0 AND NOT v_tdied THEN
            ...
            v_units:=jsonb_set(v_units,ARRAY[v_tuid,'hp'],to_jsonb(v_thp));
            -- Queue as reactive passive event
            v_react_evts:=v_react_evts||jsonb_build_array(jsonb_build_object(
              'actor',v_tuid,'hero_id','myko','skill_slot',3,
              'skill_name','Fungal Resilience','skill_type','passive','skill_dtype','physical',
              'targets',jsonb_build_array(jsonb_build_object(
                'uid',v_tuid,'heal',v_myko_heal,'hp_after',v_thp,
                'died',false,'rage_after',v_trage,'shield_after',v_tshield)),
              'rage_after',v_trage));
            END IF; -- v_myko_lv3>0
          END IF; -- myko passive check

          -- ── Clover Life Bloom — 35% reactive heal on any ally direct hit (v22) ──
          -- Uses v_dmg_pre so shield-absorbed hits also count as "direct hits"
          IF v_dmg_pre>0 AND NOT v_tdied THEN
            v_clover_uid:=NULL;
            FOR v_k2 IN SELECT k FROM jsonb_object_keys(v_units) AS t(k) LOOP
              IF (v_units->v_k2)->>'hero_id'='clover'
                 AND (v_units->v_k2)->>'side'=(v_tgt->>'side')
                 AND ((v_units->v_k2)->>'alive')::BOOLEAN
                 AND (v_units->v_k2->>'level')::INT >= 41 THEN
                v_clover_uid:=v_k2; EXIT;
              END IF;
            END LOOP;
            IF v_clover_uid IS NOT NULL AND random()<0.35 THEN
              v_clover_matk:=(v_units->v_clover_uid->>'m_atk')::INT;
              v_clover_lv3:=CASE WHEN (v_units->v_clover_uid->>'level')::INT>=240 THEN 4
                                  WHEN (v_units->v_clover_uid->>'level')::INT>=201 THEN 3
                                  WHEN (v_units->v_clover_uid->>'level')::INT>=121 THEN 2
                                  ELSE 1 END;
              v_bloom_heal:=GREATEST(1,round(v_clover_matk::NUMERIC*
                CASE v_clover_lv3 WHEN 4 THEN 0.78 WHEN 3 THEN 0.62 WHEN 2 THEN 0.50 ELSE 0.40 END));
              v_thp:=LEAST(v_tmhp,v_thp+v_bloom_heal);
              v_units:=jsonb_set(v_units,ARRAY[v_tuid,'hp'],to_jsonb(v_thp));
              v_react_evts:=v_react_evts||jsonb_build_array(jsonb_build_object(
                'actor',v_clover_uid,'hero_id','clover','skill_slot',3,
                'skill_name','Life Bloom','skill_type','passive','skill_dtype','magical',
                'targets',jsonb_build_array(jsonb_build_object(
                  'uid',v_tuid,'heal',v_bloom_heal,'hp_after',v_thp,
                  'died',false,'rage_after',v_trage,'shield_after',v_tshield)),
                'rage_after',(v_units->v_clover_uid->>'rage')::INT));
            END IF;
          END IF;
          ...
          -- ── Myko SK2 Spore Rot & ULT Spore Toxin ─────────────────────────
        ...
        -- ── HoT — Apply Heal-over-Time (Clover Lucky Toss) ───────────────────
        ELSIF v_sktype='hot' THEN
          -- No immediate heal — register HoT on target (3 turns)
          v_hot_map := jsonb_set(v_hot_map, ARRAY[v_tuid], jsonb_build_object(
            'turns', 3, 'ratio', v_skratio, 'm_atk', v_amatk
          ));
          v_tj:=v_tj||jsonb_build_array(jsonb_build_object(
            'uid',v_tuid,'heal',0,'hot_turns',3,
            'hp_after',(v_units->v_tuid->>'hp')::INT,'died',false,
            'rage_after',v_trage,'shield_after',v_tshield));

        -- ── HEAL AoE — burst heal all + HoT (Clover Bloom Cascade) ──────────
        ELSIF v_sktype='heal_aoe' THEN
          v_heal:=GREATEST(1,floor(v_amatk::NUMERIC*v_skratio));
          v_thp:=LEAST(v_tmhp,v_thp+v_heal);
          v_units:=jsonb_set(v_units,ARRAY[v_tuid,'hp'],to_jsonb(v_thp));
          -- Apply ULT HoT (3 turns, 0.38×M.ATK per turn)
          v_hot_map := jsonb_set(v_hot_map, ARRAY[v_tuid], jsonb_build_object(
            'turns', 3, 'ratio', 0.38::NUMERIC, 'm_atk', v_amatk
          ));
          v_tj:=v_tj||jsonb_build_array(jsonb_build_object(
            'uid',v_tuid,'heal',v_heal,'hot_turns',3,
            'hp_after',v_thp,'died',false,
            'rage_after',v_trage,'shield_after',v_tshield));

        -- ── BUFF / SHIELD ─────────────────────────────────────────────────────
        ELSIF v_sktype NOT IN('passive') THEN
      ...
      ELSIF v_slot=1 THEN
        ...
        -- Fang SK1 has no cooldown (fires every turn)
        IF v_ahid='fang' THEN
          v_units:=jsonb_set(v_units,ARRAY[v_auid,'sk1_cd'],to_jsonb(0));
        ELSE
      ...
      -- Lucas per-turn P.ATK stack (Warlord's Edge)
      ...
      -- ── Emit Craw Cornered Rat passive event ──────────────────────────────
      ...
      -- ── Emit reactive passive events (Myko regen, Clover Life Bloom) ──────
      FOR v_i2 IN 0 .. jsonb_array_length(v_react_evts) - 1 LOOP
        v_events := v_events || jsonb_build_array(
          jsonb_set(v_react_evts->v_i2, ARRAY['t'], to_jsonb(v_eidx)) ||
          jsonb_build_object('lucas_stacks', NULL)
        );
        v_eidx := v_eidx + 1;
        EXIT outer WHEN v_eidx >= 1500;
      END LOOP;
...
-- ── Verify ────────────────────────────────────────────────────────────────────
-- SELECT rpc_simulate_battle('1-1', '[{"hero_id":"fang","slot_index":4}]');
